...
GraphColor SGNode_SetVariable::GetNodeHeaderColor() const
{
	GraphColor col = ScriptGraphDataTypeRegistry::GetDataTypeColor(myVariable->GetTypeData()->GetType());
	col.A = 128.0f;
	return col;
}
...
GraphColor SGNode_GetVariable::GetNodeHeaderColor() const
{
	GraphColor col = ScriptGraphDataTypeRegistry::GetDataTypeColor(myVariable->GetTypeData()->GetType());
	col.A = 128.0f;
	return col;
